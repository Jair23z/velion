...
export default async function PricingPage() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-gray-900 text-white">
      ...
      {/* Hero Section */}
      <section className="pt-20 md:pt-32 pb-12 md:pb-20 px-4 md:px-8 text-center mt-5">
        <h1 className="text-3xl md:text-5xl font-bold mb-3 md:mb-4 bg-linear-to-r from-green-400 via-emerald-400 to-green-500 bg-clip-text text-transparent">
        ...
        <section className="max-w-4xl mx-auto px-8 mb-12">
          <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-lg md:text-2xl font-bold text-green-500">
                Suscripción Activa
              </h2>
            </div>
      ...
      <section className="max-w-xl mx-auto px-4 md:px-8 pb-20">
        <div className=" gap-6">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className="rounded-lg p-6 md:p-8 bg-linear-to-br from-green-900/20 to-emerald-900/20 border-2 border-green-500/30"
            >
              <div className="bg-green-600 text-white text-xs md:text-sm font-semibold px-3 py-1 rounded-full inline-block mb-3">
                PLAN ÚNICO
              </div>
              <h3 className="text-2xl md:text-4xl font-bold mb-2">{plan.name}</h3>
              <div className="mb-4">
                <span className="text-3xl md:text-5xl font-bold">${plan.price.toString()}</span>
                <span className="text-gray-300 text-sm md:text-xl">/mes</span>
              </div>
              <p className="text-gray-300 mb-6 text-base md:text-lg">{plan.description}</p>
              ...
              {session ? (
                activeSubscription?.planId === plan.id ? (
                  <button
                    disabled
                    className="w-full py-3 md:py-4 bg-gray-600 rounded-lg font-semibold cursor-not-allowed text-base md:text-lg"
                  >
                    ✓ Ya tienes este plan
                  </button>
                ) : (
